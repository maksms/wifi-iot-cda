...
void timerfunc(uint32_t timersrc)
{
	...
	if (curradio && sensors_param.cfgdes[0] != curradio && sensors_param.cfgdes[0])
	{
		sensors_param.cfgdes[0] = curradio; //сохраняем состояние
		SAVEOPT
	}
	...
	if (counter_of_time_of_free_memory > sensors_param.cfgdes[5])
		system_restart();
	if (!curradio && sensors_param.cfgdes[0] && pt2257_vol_state)
		valdes[0]++;
	if (valdes[0] > sensors_param.cfgdes[3])
	{
		os_sprintf(urlbuf, "%d", sensors_param.cfgdes[0]);
		radio_control();
		valdes[0] = 0;
	}
