...
Режим,Источник температуры,Период/сек,Гистерезис/x10,Уставка/x10,Задержка насоса/сек,Расписание,ЧЧММ-1,Уст1/x10,ЧЧММ-2,Уст2/x10,ЧЧММ-3,Уст3/x10,ЧЧММ-4,Уст4/x10,ЧЧММ-5,Уст5/x10
...
void ICACHE_FLASH_ATTR register_temp(int16_t *temp_arr, int16_t temp)
{
    uint8_t i;
...
int16_t ICACHE_FLASH_ATTR get_median_temp(int16_t *temp_arr, int16_t temp)
{
    ...
    */
    ...
    //os_sprintf(HTTPBUFF,"<div class='fll t1'>");
        ...
        if ( schedule_idx == -1 ) {
            ...
            os_sprintf(HTTPBUFF,"Уставка глобальная: <b>%d.%d °C</b><br>", (uint16_t)(global_tempset / 10), global_tempset % 10); 
        } else {
            os_sprintf(HTTPBUFF,"Уставка расписание: <b>%d.%d °C</b>", (uint16_t)(thermo->tempset / 10), thermo->tempset % 10); 
			os_sprintf(HTTPBUFF,"<br>");
        }
        ...
        os_sprintf(HTTPBUFF,"Температура%s: <b>%d.%d °C</b><br>"
                           , temp_source ? " (ext)" : ""
        ...
        os_sprintf(HTTPBUFF,"Выбег насоса (%d сек): <b>%s</b><br>", pump_active > 0 ? pump_active : pump_delay, pump_active > 0 ? "Да" : "Нет"); 
        //os_sprintf(HTTPBUFF, "</div>");  // <div class='blk'>
        #endif         
    ...
    //os_sprintf(HTTPBUFF,"<div class='flr t2'>");
        ...
        os_sprintf(HTTPBUFF,"Расписание: <br>"); 
        ...
        os_sprintf(HTTPBUFF, "<a id='ushd' href='#' data-val='%d' onclick='schd(%d,this.dataset.val)'><div class='g_%d k kk fll' id='sched' data-text='%s'>%s</div></a><br>"
                            , !schedule_enabled
    ...
    os_sprintf(HTTPBUFF,"<small>Прошивка: %s</small>", FW_VER); 
