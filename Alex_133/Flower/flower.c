...
//uart_init(9600);
valdes[0] = 0;     // предыдущее задание ШИМ
valdes[2] = 0;     // разрешение работать по времени
        if(sensors_param.cfgdes[7]){valdes[3] = 4095;}
        else{valdes[3] = 0;}
        valdes[1]=0; // задание ШИМ в %
        change_pwm();
}
...
void timerfunc(uint32_t  timersrc) {// раз 1 секунду
  if(timersrc%60==0){   // раз в минуту
    ...
    if(time_start < time_real && time_real < time_stop){  
      ...
      if(valdes[1]>0){  // плавно отключаем если задаание больше нуля
        if(sensors_param.cfgdes[7]){valdes[3] = 4095;}
        else{valdes[3] = 0;}
        valdes[1]=0;
        valdes[2]=0;
    ...
    if(sensors_param.cfgdes[6] && valdes[2]){   // если включена работа PWM
      ...
      // яркость в пределах диапазона регулирования
      ...
      // яркость больше максимума и задание больше минимума
        if(sensors_param.cfgdes[7]){valdes[3] = 4095;}
        else{valdes[3] = 0;}
        valdes[1]=0;
        change_pwm();
      }
      if(bh_l<sensors_param.cfgdes[1] && valdes[1]<100){   
      // яркость меньше минимума и задание PWM меньше максимума
    ...
    if(sensors_param.cfgdes[5]){   // если включена работа насоса     
      ...
      // при сухой земле и воде в бачке
        digitalWrite(19,1);
...
//  os_sprintf(HTTPBUFF,"Кнопки %d <br>",valdes[2]);
...
//  os_sprintf(HTTPBUFF,"<script>function pt(x){(document.createElement('img')).src=(location.href+'valdes?int=2&set='+x);}</script>");
  if(valdes[2]){
  ...
  if(sensors_param.cfgdes[4]){
    ...
    os_sprintf(HTTPBUFF,"<a><div class='g_1 k fll' style='width:120px;background:grey'>Зелёный вкл.</div></a><br>");  // <script>function pt(x){(document.createElement('img')).src=(location.href+'valdes?int=2&set='+x);}</script>
  }
...
// sensors_param.cfgdes[8] * sensors_param.cfgdes[9] < time_loc.hour * time_loc.min < sensors_param.cfgdes[10] * time_loc.min<sensors_param.cfgdes[11] 
//  os_sprintf(HTTPBUFF," %d %d %d %d %d %d <br>",sensors_param.cfgdes[8], sensors_param.cfgdes[9], time_loc.hour, time_loc.min, sensors_param.cfgdes[10], time_loc.min<sensors_param.cfgdes[11]);
  ...
  //uint32_t timerall=timer_uptime.day*3600*24+timer_uptime.hour*3600+timer_uptime.min*60+timer_uptime.sec;
