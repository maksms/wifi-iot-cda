//при сборке добавить 9 настроек , 1 переменную
//кнопкой принять сохраняются настройки,кнопка режим.режим работы гпио- постоянно выкл,постоянно вкл,,охладитель.термостат.настройки на главной модуля
//valuedes0 режим по мктт
int32_t tempVhod;
bool time = 0;
uint8_t	gpio = 16; //номер гпио выход
void ICACHE_FLASH_ATTR startfunc() 
{
	valdes[0] = sensors_param.cfgdes[8];
}
void ICACHE_FLASH_ATTR timerfunc(uint32_t timersrc)
{
	...
	if( valdes[0] != sensors_param.cfgdes[8] )
    {
        if( timersrc > 10 )
	    {
			sensors_param.cfgdes[8] = valdes[0];
			SAVEOPT
		}	
		else
			valdes[0] = sensors_param.cfgdes[8];
    ...
    if ( sensors_param.cfgdes[8] == 1 ){ //холод
        if ( time == 1 ) {
            if ( sensors_param.cfgdes[1] < tempVhod )
				digitalWrite(gpio,1);
				if ( tempVhod < sensors_param.cfgdes[0] )
				digitalWrite(gpio,0);
        }
		else
		{
            if ( sensors_param.cfgdes[5] < tempVhod )
				digitalWrite(gpio,1);
            if ( tempVhod < sensors_param.cfgdes[4] )
				digitalWrite(gpio,0);
        }
    }
    ...
    if ( sensors_param.cfgdes[8] == 3 )
		digitalWrite(gpio,1);//вкл постоянно
}
void webfunc(char *pbuf)
{ //тут обьяснялок не будет
